...
function updateLocker(mysqli $db, string $lockerId, int $studentNo, string $status): bool {
    $stmt = $db->prepare("UPDATE lockers SET Status=?, StudentSchoolNumber=? WHERE LockerID=?");
...
function updateBooking(mysqli $db, string $bookingId, string $status, string $lockerId): bool {
    $stmt = $db->prepare("UPDATE bookings SET Status=?, LockerID=? WHERE BookingID=?");
    if (!$stmt) return false;
...
function updateWaitingListById(mysqli $db, string $waitingId, string $status, int $adminId, string $lockerId): bool {
    $stmt = $db->prepare("UPDATE waitinglist SET Status=?, AdminID=?, LockerID=? WHERE WaitingListID=?");
    if (!$stmt) return false;
...
function applyForLocker(
    ...
    $bookedOnDate = date('Y-m-d'); // current date in YYYY-MM-DD format
        ...
        ");
        if (!$stmtW) throw new Exception("Waitinglist insert prepare failed: ".$mysqli->error);

        $stmtW->bind_param("sii", $bookingId, $studentNo, $parentId);
        if (!$stmtW->execute()) throw new Exception("Waitinglist insert failed: ".$stmtW->error);
        $stmtW->close();
