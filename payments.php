...
require_once __DIR__ . '/../includes/parentmailer.php';
...
function updatePaymentStatus(mysqli $mysqli, string $paymentId, string $status, string $adminId, array $adminEmails): array {
    ...
    try {
        // 1. Check current payment row
        $stmt = $mysqli->prepare("
            SELECT PaymentStatus, BookingID, StudentSchoolNumber
            FROM payments
            WHERE PaymentID=?
        ");
        ...
        if (!$payment) {
            throw new Exception("Payment not found for PaymentID $paymentId");
        }
        if ($payment['PaymentStatus'] === $status) {
            throw new Exception("Payment $paymentId is already $status");
        }
        ...
        $studentName = $info['StudentName']." ".$info['StudentSurname'];
        $parentName  = $info['ParentName']." ".$info['ParentSurname'];
        ...
        if ($status === 'Verified') {
            sendMail(
                ...
                "<p>Dear $parentName,</p>
                 ...
                 Your child $studentName is now in the allocation queue.</p>",
                "Dear $parentName,\n\nYour payment for booking {$info['BookingID']} has been verified. 
                 Your child $studentName is now in the allocation queue."
            );
        } else {
            sendMail(
                $parentEmail,
        ...
        $success = "Payment $paymentId updated to $status and notifications sent.";
    } catch (Throwable $txe) {
        $mysqli->rollback();
        error_log("Update payment failed: ".$txe->getMessage());
        $error = $txe->getMessage(); // show specific reason instead of generic
    }
...
$submittedPayments = $mysqli->query("
    SELECT 
    pay.PaymentID,          -- use this in your hidden form field
    b.BookingID,            -- show this in the table for admins
    s.StudentName, 
    s.StudentSurname, 
    s.StudentGrade,
    p.ParentName, 
...
JOIN parents p 
    ON b.ParentID = p.ParentID
WHERE pay.PaymentStatus = 'Submitted'
    ...
    <?php if ($submittedPayments && $submittedPayments->num_rows > 0): ?>
      <?php while ($row = $submittedPayments->fetch_assoc()): ?>
        <tr>
          ...
          <td><?= !empty($row['PaymentDate']) ? safe($row['PaymentDate']) : '—' ?></td>
          <td>
            <?php if (!empty($row['ProofOfPaymentFile'])): ?>
              <a href="uploads/<?= htmlspecialchars($row['ProofOfPaymentFile']) ?>" target="_blank" class="btn btn-sm btn-outline-primary">View</a>
              <a href="uploads/<?= htmlspecialchars($row['ProofOfPaymentFile']) ?>" download class="btn btn-sm btn-outline-secondary">Download</a>
            <?php else: ?>
              —
            <?php endif; ?>
          </td>
