<?php
session_start();
require_once __DIR__ . '/../includes/dbconnect.php';   // sets up $mysqli
require_once __DIR__ . '/../includes/functions.php';   // defines helper functions
require_once __DIR__ . '/../includes/parentmailer.php';// defines mail functions
// Admin session check
...
if (isset($_POST['allocate']) && !empty($_POST['waiting_id'])) {
    ...
    if (!$stmt) {
        ...
        $stmt->bind_param("s", $waitingId); // string binding
        $stmt->execute();
        ...
        if ($entry) {
            ...
            $studentName = $entry['StudentName'].' '.$entry['StudentSurname'];
            ...
            try {
                ...
                if (updateWaitingListById($mysqli, $waitingId, 'Allocated', $admin_id, $lockerId)) throw new Exception("Waitinglist update failed");
                $mysqli->commit();

                // Fetch details after commit
                $stmt = $mysqli->prepare("
                SELECT s.StudentName, s.StudentSurname, s.StudentGrade,
                 p.ParentName, p.ParentSurname, p.ParentEmail,
                b.BookingID, l.LockerID
                FROM bookings b
              JOIN students s ON s.StudentSchoolNumber = b.StudentSchoolNumber
              JOIN parents p  ON p.ParentID = b.ParentID
              LEFT JOIN lockers l ON l.LockerID = b.LockerID
              WHERE b.BookingID = ?
              ");
    ...
    if (!$details) throw new Exception("No allocation details found for BookingID {$bookingId}");

    $studentFirst   = $details['StudentName'];
    $studentSurname = $details['StudentSurname'];
    $studentGrade   = $details['StudentGrade'];
    $parentName     = $details['ParentName'];
    $parentSurname  = $details['ParentSurname'];
    $parentEmail    = $details['ParentEmail'];
    $lockerId       = $details['LockerID'];

    $studentFull = $studentFirst.' '.$studentSurname;

    // Parent email
    $htmlBody = "Dear {$parentName} {$parentSurname},<br><br>
    Your child <strong>{$studentFull}</strong> (Grade {$studentGrade})
    ...
    Your child {$studentFull} (Grade {$studentGrade})
...
<nav class="navbar navbar-expand-lg glass-nav fixed-top">
  <div class="container-fluid">
    <a class="navbar-brand fw-bold text-dark" href="#">Amandla High Admin</a>
